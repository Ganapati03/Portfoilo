...
export class MzaCarousel {
  ...
  _measure() {
    if (!this.viewport || !this.pagination || !this.root) return;
  ...
  _onDragMove(e) {
    ...
    if (Math.abs(dx) > 5) this.state.dragMoved = true;
  ...
  _onDragEnd(e) {
    ...
    try {
      if (this.state.pointerId != null && this.viewport)
